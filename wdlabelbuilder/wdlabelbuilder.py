...
class WDLabelBuilder():
    ...
    def __init__(self, args):
        ...
        if sys.version_info.major > 2:
            self.qnumber_title = args.qtitle
            self.label_title = args.ltitle
            self.prefix = args.prefix
            self.suffix = args.suffix
        else:
            self.qnumber_title = unicode(args.qtitle, encoding='utf-8') 
            self.label_title = unicode(args.ltitle, encoding='utf-8')
            self.prefix = unicode(args.prefix, encoding='utf-8')
            self.suffix = unicode(args.suffix, encoding='utf-8')
...
def process_arguments(input_args=None):
    ''' Generate help page and process user arguments. '''
    parser = argparse.ArgumentParser(
        description='Create labels, descriptions and aliases for Wikidata items '
        + 'based on existing labels for those items.')
    parser.add_argument(
        'language', help='Language of the label/alias/description you are adding. '
        + 'Use Wikimedia abbreviations (e.g. "en", "fr").')
    parser.add_argument(
    ...
    type_group.add_argument(
        '-d', '--description', action='store_true', help='Generate descriptions')
    parser.add_argument(
        '-t', '--timeseries', action='store_true', 
        help='Use numbers, such as years from labels in the input json '
        + 'to create labels in another language.')
    parser.add_argument(
        '-p', '--prefix', default='',
    ...
    parser.add_argument(
        ...
        help='Title of the label field in the json file. Default is "itemLabel".')
    output_group = parser.add_mutually_exclusive_group()
    output_group.add_argument(
        '-j', '--json', action='store_true',
        help='Export results as a json instead of QuickStatements commands.')
    output_group.add_argument('-u', '--url', action='store_true',
    ...
    if not args.label and not args.alias and not args.description:
        raise parser.error('Missing required value: label, alias or description.')
    qs_file = WDLabelBuilder(args)
